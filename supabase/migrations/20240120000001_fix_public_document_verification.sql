-- Migration: Allow public document verification and tracking
-- Enables public read on document_requests and creates secure lookup RPC function

-- 1. Ensure RLS SELECT policy exists for public document tracking and verification
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
      ...
      AND policyname = 'Public document verification and tracking'
  ) THEN
    CREATE POLICY "Public document verification and tracking"
      ON public.document_requests FOR SELECT
      USING (true);
  END IF;
END $$;

-- 2. Create high-performance RPC function for document verification lookup
...
BEGIN
  ...
  WHERE 
    ...
    OR (dr.control_number ILIKE '%' || v_trimmed || '%')
    OR (UPPER(SUBSTRING(dr.id::text, 1, 8)) = UPPER(REPLACE(REPLACE(v_trimmed, 'BD1-', ''), 'BD2-', '')))
