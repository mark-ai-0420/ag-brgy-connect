...
const ROUTES = [
  ...
  { name: 'Officials Roster', path: '/officials' },
  { name: 'Sign In', path: '/auth/sign-in' },
];

const PORT = 3008;
const BASE_URL = `http://localhost:${PORT}`;
const ARTIFACT_DIR = path.join(process.cwd(), 'scratch/lighthouse');
...
async function runLighthouseAudit() {
  console.log('🚀 Starting Comprehensive Production Lighthouse Audit Suite on Port 3008...');
  ...
  for (const r of ROUTES) {
    await checkServer(`${BASE_URL}${r.path}`);
  }
  ...
  for (const route of ROUTES) {
    ...
    const jsonOutputPath = path.join(ARTIFACT_DIR, `lighthouse_${route.name.toLowerCase().replace(/\s+/g, '_')}.json`);
  ...
  // Save summary JSON
  const summaryPath = path.join(ARTIFACT_DIR, 'lighthouse_master_summary.json');
